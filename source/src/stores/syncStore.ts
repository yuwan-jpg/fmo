...
export const useSyncStore = defineStore('sync', () => {
  ...
  // 每次打开应用都强制执行一次全量同步：该标志在页面加载（store 初始化）时为 true，
  // 首次自动同步（此时已有已添加地址）执行后置为 false，本次会话内不再重复全量同步
  let startupFullSyncPending = true

  // 上次"全量/今日"同步时间：持久化到 localStorage，
  // 用于控制本次会话中定时同步的节奏（每 10 秒一次轮询不重复全量同步）
  ...
  function startAutoSyncTask(getAddresses: () => SyncAddress[]) {
    ...
    autoSyncTimer = setInterval(async () => {
      ...
      try {
        ...
        // 每次打开应用（已添加地址）都先强制执行一次全量同步；
        // 数据库为空（首次/数据被清）时无论如何都要先同步补齐；
        // 否则仅当距离上次"今日/全量"同步超过 1 小时才执行，避免定时器每 10 秒重复全量同步
        const dbEmpty = !ctx.getDbLoaded?.() || (ctx.getTotalLogs?.() || 0) === 0
        const needsFullSync = startupFullSyncPending || dbEmpty || now - lastFullSyncTime >= oneHour
        ...
        if (needsFullSync) {
          startupFullSyncPending = false
          saveLastFullSyncTime(now)
        }
      } catch (err) {
